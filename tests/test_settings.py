...
class TestSettingsDefinitions:
    ...
    def test_expected_settings_exist(self):
        """Test that expected MCP settings are defined."""
        keys = [d.key for d in SETTINGS_DEFINITIONS]
        assert "mcp.enabled_tools" in keys
        assert "mcp.config_url_style" in keys
...
class TestSettingsServiceCache:
    ...
    def test_cache_hit(self, service):
        ...
        cache_key = service._make_cache_key("server.port")
    ...
    def test_cache_expiry(self, service):
        ...
        with patch.dict(os.environ, {"JARVIS_MCP_PORT": "9999"}):
            result = service.get("server.port")
            assert result == 9999
...
class TestSettingsServiceEnvFallback:
    ...
    def test_env_fallback_when_db_unavailable(self, service):
        ...
        with patch.dict(os.environ, {"JARVIS_MCP_PORT": "9000"}):
            result = service.get("server.port")
            assert result == 9000
...
class TestSettingsServiceTypedGetters:
    ...
    def test_get_int(self, service):
        """Test get_int method."""
        with patch.dict(os.environ, {"JARVIS_MCP_PORT": "9000"}):
            result = service.get_int("server.port", 0)
            assert result == 9000
            assert isinstance(result, int)

    def test_get_str(self, service):
        """Test get_str method."""
        with patch.dict(os.environ, {"JARVIS_MCP_HOST": "0.0.0.0"}):
            result = service.get_str("server.host", "")
...
class TestSettingsServiceListMethods:
    ...
    def test_list_all_with_category_filter(self, service):
        ...
        settings = service.list_all(category="mcp")

        assert all(s["category"] == "mcp" for s in settings)
...
class TestSingleton:
    ...
    def test_singleton_instance(self):
        ...
        with patch("jarvis_mcp.db.session.get_session_local", return_value=mock_session_local):
            with patch("jarvis_mcp.db.models.Setting", mock_setting):
